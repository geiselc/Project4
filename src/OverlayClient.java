...
public class OverlayClient {
	...
	private class Write extends Thread {
		...
		public void buildPacket(String message, String dstIP) {
			...
			udpHead.setSrcPort("0010011010010000"); // 9876
			udpHead.setDstPort("0010011010010000"); // 9876
		...
		private String udpCSum() {
			...
			/*
			 * for (int i = 0; i < fin.length; i++) { String str =
			 * Byte.toString(fin[i]); }
			 */
			byte[] inputData = fin;
			long FF00 = 0xff00;
			...
			while (length > 1) {
				...
				sum += data;
				// 1's complement carry bit correction in 16-bits (detecting
				// sign
				// extension)
				if ((sum & 0xFFFF0000) > 0) {
					sum = sum & 0xFFFF;
			 ...
			 * temp = "" + Integer.toBinaryString(a); add = binaryAdd(add,
			 * temp); System.out.println(temp); a =
			 * (Integer.parseInt(ipHead.srcAddress, 2)) & 0xFFFF; temp = "" +
			 * Integer.toBinaryString(a); add = binaryAdd(add, temp);
			 * System.out.println(temp); a =
			 * (Integer.parseInt(ipHead.dstAddress, 2) >>> 16) & 0xFFFF; temp =
			 * "" + Integer.toBinaryString(a); add = binaryAdd(add, temp);
			 * System.out.println(temp); a =
			 * (Integer.parseInt(ipHead.dstAddress, 2)) & 0xFFFF; temp = "" +
			 * Integer.toBinaryString(a); add = binaryAdd(add, temp);
			 * System.out.println(temp); a =
			 * (Integer.parseInt("00000000"+ipHead.protocol, 2)); temp = "" +
			 * Integer.toBinaryString(a); add = binaryAdd(add, temp);
			 * System.out.println(temp); a = (Integer.parseInt(udpHead.length,
			 * 2)); temp = "" + Integer.toBinaryString(a); add = binaryAdd(add,
			 * temp); System.out.println(temp); a =
			 * (Integer.parseInt(udpHead.srcPort, 2)); temp = "" +
			 * Integer.toBinaryString(a); add = binaryAdd(add, temp);
			 * System.out.println(temp); a = (Integer.parseInt(udpHead.dstPort,
			 * 2)); temp = "" + Integer.toBinaryString(a); add = binaryAdd(add,
			 * temp); System.out.println(temp); a =
			 * (Integer.parseInt(udpHead.length, 2)); temp = "" +
			 * Integer.toBinaryString(a); add = binaryAdd(add, temp);
			 * System.out.println(temp); while (add.length() > 16) { String low
			 * = add.substring(add.length()-16); String high = add.substring(0,
			 * add.length()-16); add = binaryAdd(low, high); } while
			 * (add.length() < 16) { add = "0" + add; } for (int j = 0; j <
			 * add.length(); j++) { if (add.charAt(j) == '0') { result += "1"; }
			 * else { result += "0"; } }
			 */
			// return result;
		}
	...
	private class Read extends Thread {
		...
		public void buildIP(byte[] data) {
			...
			ipHead.setSrcAddress(byteThirteen + byteFourteen + byteFifteen
					+ byteSixteen);
			...
			ipHead.setDstAddress(byteSeventeen + byteEighteen + byteNineteen
					+ byteTwenty);
		}
